...
from bot_commands import BOT_MENU_COMMANDS
...
async def _sync_bot_menu_commands(telegram_app: Application) -> None:
    try:
        ...
        LOGGER.warning("Failed to sync Telegram bot commands: %s", err)
        return
...
def build_app(settings: Settings) -> tuple[FastAPI, Optional[Application]]:
    ...
    async def lifespan(_: FastAPI):
        ...
        bridge.bind_application(telegram_app)
        await bridge.scheduler_service.start(telegram_app)
        ...
        try:
            ...
            await bridge.scheduler_service.stop()
            bridge.bind_application(None)
