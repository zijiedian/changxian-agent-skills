...
- `/setting memory on|off`
- `/setting scheduler on|off`

补充：

- 发送普通文本（非 `/xxx`）即触发任务执行
- 记忆系统不再通过 `/memory` 命令管理，而是由内置 skill `changxian-memory-manager` 自动加载与维护
...
- `TG_ENABLE_MEMORY`（默认 `1`；启用 skill 驱动的记忆加载与自动维护）
- `TG_MEMORY_MAX_ITEMS` / `TG_MEMORY_MAX_CHARS`（控制每次注入的记忆数量和长度）
- `TG_ENABLE_SCHEDULER`（默认 `1`；启用后台定时任务轮询）
- `TG_SCHEDULER_POLL_SECONDS`（默认 `5`；后台调度轮询间隔）
- `TG_DEFAULT_TIMEZONE`（默认 `Asia/Shanghai`；定时任务解析与展示时区）
