// ============================================================================
// TelegramRenderer - Telegram 平台特定渲染器
// 参考 OpenACP/src/plugins/telegram/renderer.ts
// ============================================================================
...
function appendElapsedFooter(bodyHtml, elapsedText) {
  const footer = `<i><code>${elapsedText}</code></i>`;
...
export class TelegramRenderer extends BaseRenderer {
  ...
  renderText(content, verbosity) {
    return {
      body: markdownToTelegramHtml(content.text),
      format: 'html',
    };
  }
  ...
  renderPlan(content) {
    ...
    const lines = entries.map((e, i) => {
      ...
      return `${icon} ${i + 1}. ${escapeHtml(e.content)}${priority}`;
    });
    return {
      body: `📋 <b>Plan</b>\n${lines.join('\n')}`,
      format: 'html',
  ...
  renderUsage(content, verbosity) {
    ...
    if (!meta.tokensUsed) {
      return { body: '📊 <i>Usage data unavailable</i>', format: 'html' };
    }

    const costStr = meta.cost != null ? ` · <b>$${meta.cost.toFixed(2)}</b>` : '';

    if (verbosity === 'medium') {
      return {
        body: `📊 <b>${meta.tokensUsed}</b> tokens${costStr}`,
        format: 'html',
      };
    }
    ...
    if (!meta.contextSize) {
      return {
        body: `📊 <b>${meta.tokensUsed}</b> tokens`,
        format: 'html',
      };
    }
    ...
    let text = `📊 <b>${meta.tokensUsed}</b> / <b>${meta.contextSize}</b> tokens\n${pct}%`;
    if (meta.cost != null) text += `\n💰 <b>$${meta.cost.toFixed(2)}</b>`;
  ...
  renderSessionEnd(content) {
    return {
      body: `🔚 <b>Session ended:</b> ${escapeHtml(content.text)}`,
      format: 'html',
    };
  }

  // ============================================================================
  // 遗留兼容方法 - 用于 renderTelegramPayload
  // ============================================================================
  ...
  renderLegacyPayload(payload) {
    ...
    if (status === 'Running' && (!hasProgressDetails && (marker === 'thinking' || previewLower === 'thinking...' || previewLower.startsWith('thinking\n') || !preview))) {
      ...
      return { html: body, pages: [body], images: [] };
    }
    ...
    if (status === 'Done') {
      ...
      const pages = this.buildTelegramStructuredPages(previewForPages);
      return {
    ...
    if (heading) fallbackLines.push(`<b>${escapeHtml(heading)}</b>`);
    if (previewModel.summary) fallbackLines.push(escapeHtml(previewModel.summary));
    const resolvedProgressMarkdown = progressMarkdown || fallbackLines.join('\n').trim() || '请稍候。';
    const pages = this.buildTelegramProgressPages(resolvedProgressMarkdown, elapsedText);
    return {
      html: pages[0] || '<i>暂无输出</i>',
      pages,
      images: media.images,
  ...
  buildTelegramPages(text, pageLimit = TELEGRAM_FINAL_PAGE_LIMIT) {
    ...
    while (changed) {
      ...
      for (const page of pages) {
        const html = this.coerceTelegramHtml(page);
    ...
    return pages.map((page) => this.coerceTelegramHtml(page));
  }

  buildTelegramProgressPages(text, elapsedText) {
    const pages = this.buildTelegramPages(text, TELEGRAM_RUNNING_PAGE_LIMIT);
    const resolved = pages.length ? pages : ['<i>暂无输出</i>'];
    return resolved.map((page) => appendElapsedFooter(page, elapsedText));
  }

  buildTelegramStructuredPages(preview) {
    const pages = [];
    ...
    const normalizedPages = pages.length ? pages : ['暂无输出'];
    return normalizedPages.map((page) => this.coerceTelegramHtml(page));
  }
