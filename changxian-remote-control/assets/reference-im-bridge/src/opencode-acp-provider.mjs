...
class TelegramAcpClient {
  ...
  async requestPermission(params) {
    ...
    if (payload) {
      await this.context.onProgress?.(payload);
    }
    return option
...
export class OpencodeAcpProvider {
    ...
    if (!this.initPromise) {
      this.initPromise = (async () => {
        try {
          ...
          throw new Error(`OpenCode ACP SDK init failed: ${detail}`);
        }
      })();
    }
  ...
  async runTask(params) {
    ...
    if (!args.length) {
      throw new Error('OpenCode ACP command prefix is empty');
    }
    ...
    const connection = new acp.ClientSideConnection(() => new TelegramAcpClient(this, context), stream);
    ...
    try {
      ...
      this.diagnostics = {
        ...this.diagnostics,
        initialized: true,
        agentName: String(init?.agentInfo?.name || 'OpenCode'),
      ...
      const authHint = /auth/i.test(detail) || /auth/i.test(stderrSummary)
        ? ' Run `opencode auth login` or check your OpenCode provider credentials.'
        : '';
      this.diagnostics = {
        ...this.diagnostics,
        initialized: false,
      ...
      throw new Error(`OpenCode ACP run failed: ${detail}${stderrSummary ? `\n${stderrSummary}` : ''}.${authHint}`.trim());
    } finally {
